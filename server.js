...
const start = () => {
    ...
    .then((answer) => {
        ...
        if (answer.action == "View total budget of a department") {
            totalBudget();
        }
    ...
    inquirer.prompt(employeeQuestions).then((answer) => {
        ...
        .then((input) => {
            ...
            .then((input) => {
                ...
                let manager_id = employees[index].gertID();
...
var deletedEmployee = () => {
    let employee = [];
    for (emp of employees) {
        employee.push(emp.getFristName() + " " + emp.getLastName());
    }
    inquirer
    .prompt({
        name: "name",
